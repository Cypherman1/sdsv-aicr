  ...
  if (m.has(family)) {
    ...
    return m.set(
      ...
      _extends({}, origFont, {
        ...
        subFamilies: [].concat(_toConsumableArray(origFont.subFamilies), [
          subFamily
        ]),
        files: _extends(
          {},
          origFont.files,
          _defineProperty({}, subFamily, file)
        ),
        postscriptNames: _extends(
          {},
          origFont.postscriptNames,
          _defineProperty({}, subFamily, postscript)
        )
      })
    );
  } else {
...
var SystemFonts = function SystemFonts() {
  ...
  var _options$ignoreSystem = options.ignoreSystemFonts,
    ignoreSystemFonts =
      _options$ignoreSystem === undefined ? false : _options$ignoreSystem,
  ...
  var getFontFiles = function getFontFiles() {
    ...
    if (platform === "osx") {
      ...
      directories = [].concat(_toConsumableArray(directories), [
        _path2.default.join(home, "Library", "Fonts"),
        _path2.default.join("/", "Library", "Fonts"),
        _path2.default.join("/", "System", "Library", "Fonts")
      ]);
    } else if (platform === "windows") {
      var winDir = process.env.windir || process.env.WINDIR;
      directories = [].concat(_toConsumableArray(directories), [
        _path2.default.join(winDir, "Fonts")
      ]);
    } else {
      // some flavor of Linux, most likely
      var _home = process.env.HOME;
      directories = [].concat(_toConsumableArray(directories), [
        _path2.default.join(_home, ".fonts"),
        _path2.default.join(_home, ".local", "share", "fonts"),
        _path2.default.join("/", "usr", "share", "fonts"),
        _path2.default.join("/", "usr", "local", "share", "fonts")
      ]);
    }
  ...
  this.getFontsExtended = function() {
    return new Promise(function(resolve, reject) {
      ...
      var filteredFontFiles = !ignoreSystemFonts
        ? [].concat(_toConsumableArray(fontFiles))
          ...
          new Promise(function(resolve1) {
            _ttfinfo2.default.get(file, function(err, fontMeta) {
              if (!fontMeta) {
                ...
                resolve1(
                  tableToObj(
                    fontMeta.tables.name,
                    file,
                    !customFontFiles.has(file)
                  )
                );
              }
            });
      ...
      Promise.all(promiseList).then(
        function(res) {
          ...
          var namesArr = []
            .concat(_toConsumableArray(names.values()))
            .sort(function(a, b) {
  ...
  this.getFontsExtendedSync = function() {
    ...
    var names = filteredFontFiles
      .reduce(function(arr, file) {
        ...
        return arr.concat([
          tableToObj(data.tables.name, file, !customFontFiles.has(file))
        ]);
      }, [])
      .filter(function(data) {
        return data ? true : false;
